...
export default function App() {
  ...
  const refreshLabel = researchQueued ? "推进已排队" : "推进";
  ...
  return <div onClick={openLocalFile} className={`workspace ${chatOpen && !onboarding && page !== "background" ? "chat-open" : ""} ${onboarding ? "onboarding-workspace" : ""}`}>
    ...
    <main className="main"><header className="topbar"><div className="breadcrumb"><span>我的工作空间</span><span>/</span><h1>{titles[page]}</h1>{project && <span>/ {project}</span>}</div><select className="mobile-navigation" aria-label="页面导航" value={page} onChange={(event) => navigate(event.target.value as Page)}>{Object.entries(titles).map(([key,value]) => <option key={key} value={key}>{value}</option>)}</select><div className="topbar-actions">{page !== "background" && <><div className="advance-control"><AdvanceSchedule research={state.research} paused={state.paused} connected={connected} running={researching} queued={researchQueued} unavailable={query.error ? "正在恢复与本地服务的连接。" : state.runtime.fileIssues.length ? "工作台文件需要修正，请查看页面提示。" : state.runtime.error || undefined}/>{researching ? <div className="button button-brandSubtle button-sm advance-running" role="group" aria-label="工作推进">
          <button type="button" className="advance-stop" aria-label={stopping ? "正在停止推进" : "停止推进"} title="停止本轮工作推进" disabled={mutation.isPending || stopping} onClick={() => void stopResearch()}>{stopping ? <LoaderCircle size={12} className="refresh-spinning"/> : <Square size={10} fill="currentColor"/>}</button>
          <span role="status">{stopping ? "正在停止…" : "正在推进"}</span>
        </div> : <Button variant="brandSubtle" size="sm" aria-label={refreshLabel} title="查看全局，并按当前计划推进工作" disabled={mutation.isPending || researchQueued || state.reviewMode !== "batch"} onClick={() => void startResearch()}>{researchQueued ? <LoaderCircle size={15} className="refresh-spinning"/> : <Play size={15}/>} {refreshLabel}</Button>}</div><Button variant={chatOpen ? "brandSubtle" : "ghost"} size="sm" aria-expanded={chatOpen} onClick={toggleWorkChat}><MessageSquare size={15}/>和 AI 聊聊</Button><Button size="sm" onClick={() => {setChatOpen(true);setComposeRequest(composeRequest+1);}}><Plus size={14}/>新建事项</Button></>}</div></header>
